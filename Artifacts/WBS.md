...
| 7.3 | `[F]` Add/edit entry form → encrypts before send | 7.1, 6.2 | Done |
| 7.4 | `[F]` Delete confirmation flow | 7.1 | Done |
| 7.5 | `[F]` Search + copy-to-clipboard on entry | 7.2 | Done |
| 7.6 | `[F]` Wire list/add/edit/delete to CRUD endpoints (3.1–3.4) | 7.1–7.4 | Done |
...
| 9.2 | `[F]` Delta fetch (`GET /entries?since=`) + `last_synced_at` persistence | 9.1, 3.4 | Done |
| 9.3 | `[F]` LWW merge logic: tombstone → remove; server newer → overwrite; local newer → queue push | 9.2 | Done |
| 9.4 | `[F]` Push queue: pending `is_pending_sync=1` rows → `POST`/`PUT`/`DELETE`, clear flag on success | 9.3, 3.1–3.3 | Done |
| 9.5 | `[F]` "Sync Now" manual button wired to the same engine | 9.1 | Done |
...
| 12.4 | `[I]` Cross-origin CORS re-verified against actual Web deploy domain | 4.1 | Done |
...
| 13.3 | `[F]` Build Android release APK | 12.x | Done |
